...
def start(update: Update, context):
    ...
    update.message.reply_text(f'Добрый день, {user.full_name}\!\nЧтобы войти в режим *Мониторинга системы*, воспользуйтесь командой /monitoring\.\nДля работы с *текстом* воспользуйтесь командой /text', parse_mode='MarkdownV2')


def helpCommand(update: Update, context):
    update.message.reply_text('*Help*', parse_mode='MarkdownV2')
...
def monitoring(update: Update, context):
    update.message.reply_text('Вы вошли в режим *Мониторинга* системы\.', parse_mode='MarkdownV2')
...
def text(update: Update, context):
    update.message.reply_text('Вы вошли в режим *Работы с текстом*\.\nКоманда /findPhoneNumbers - поиск телефонных номеров;\nКоманда /findEmails - поиск адресов электронной почты\.', parse_mode="MarkdownV2")
...
def get_ss(update: Update, context):
    ...
    client.connect(hostname=host, username=userkali, password=passkali, port=port)
    stdin, stdout, stderr = client.exec_command('netstat -tuln')
    data = stdout.read() + stderr.read()
    client.close()
    data = str(data).replace('\\n', '\n').replace('\\t', '\t')[2:-1]
    update.message.reply_text(data)
...
def verifyPassword(update: Update, context):
    ...
    if password_regex.match(user_input):
        update.message.reply_text("<code>/get_release</code>Пароль сложный.")
    else:
